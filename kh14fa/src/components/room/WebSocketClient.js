...
const WebSocketClient = ()=>{
    ...
    const connectToServer = useCallback(()=>{
        ...
        const client = new Client({
            ...
            onConnect: ()=>{
                ...
                if(login === true) {//회원
                    client.subscribe("/public/dm/"+memberId, (message)=>{
                        const json = JSON.parse(message.body);
                        setMessageList(prev=>[...prev, json]);//순서 보장
                    });
                    ...
                    client.subscribe("/public/db", (message)=>{
                        const json = JSON.parse(message.body);
                        setMessageList(json.messageList);//덮어쓰기
                        setMore(json.last === false);//더보기 여부 설정
                    });
    ...
    const sendMessage = useCallback(()=>{
        ...
        if(connect === false) return;//연결이 안된 경우 차단
        if(input.length === 0) return;//입력값이 없는 경우 차단

        //DM 발송 조건 - `/w` 또는 `/ㅈ`로 시작할 것
        ...
        const message = {
            destination: "/app/chat",//보내는 목적지(서버의 설정에 따름)
    ...
    const sendDM = useCallback(()=>{
        ...
        const message = {
            destination: "/app/dm/"+receiverId,//보내는 목적지(서버의 설정에 따름)
            body: JSON.stringify(json),//JSON 문자열로 전송
            headers:{
                accessToken : accessToken,
                refreshToken : refreshToken
    ...
    //`비동기`로 더보기 기능 구현
    //- 회원이면 이 요청의 헤더에 Authorization이 포함된다
